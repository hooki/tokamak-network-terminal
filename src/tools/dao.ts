...
import { z } from 'zod';
import { readContract, readContracts, writeContract, getAccount, getPublicClient } from '@wagmi/core';
...
export function registerDAOTools(server: McpServer) {
      ...
      try {
        ...
        const memberInfo = members.map(member => ({
          ...
          candidateInfo: member.candidateInfo ? {
            ...
            memberJoinedTime: member.candidateInfo.memberJoinedTime.toString(),
            rewardPeriod: member.candidateInfo.rewardPeriod.toString(),
            claimedTimestamp: member.candidateInfo.claimedTimestamp.toString(),
          } : null,
        }));
    ...
    async ({ network = 'mainnet', candidateContract, claim = false }) => {
      ...
      try {
        ...
        if (claim) {
          ...
          const walletCheck = await checkWalletConnection(undefined, callbackCommand);
  ...
  server.registerTool(
    'get-challenge-member-info',
    {
      ...
      description: 'Get information about challenging a DAO committee member, including stake requirements and eligibility.',
      inputSchema: {
        memberIndex: z
          .number()
    ...
    async ({ memberIndex, challengerCandidate, network = 'mainnet' }) => {
      try {
        ...
        const challengeInfo = await getChallengeInfo(memberIndex, challengerCandidate, network);
  ...
  server.registerTool(
    'execute-challenge-member',
    {
      ...
      description: 'Execute a challenge against a DAO committee member. Requires wallet connection and proper authorization.',
      inputSchema: {
        memberIndex: z
          .number()
          .describe('The index of the current DAO member slot to challenge'),
      ...
      try {
        ...
        if (String(operator).toLowerCase() !== account.address?.toLowerCase()) {
          return createErrorResponse({
            ...
            message: 'Connected wallet is not the authorized operator for this challenger candidate.'
          });
        }
